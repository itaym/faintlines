export const messagesAutoRun = [
    {
        button: 'CONTINUE',
        text: 'Roger is released from jail thanks to the proof you found in his phone!',
        title: 'YOU DID IT!',
    },
    {
        button: '1/5, CONTINUE TO 2',
        text: 'A very good day to you! I must admit, it was a very challenging assignment.',
        title: 'Hi Tzach!',
    },
    {
        button: '2/5, CONTINUE TO 3',
        text: 'Those auto-generated messages are entering the queue each two seconds to demonstrate the queue feature.',
        title: 'THE QUEUE!',
    },
    {
        button: '3/5, CONTINUE TO 4',
        text: 'One feature of that challenging UI was beyond my supernatural powers. I WALK IN SHAME.',
        title: 'I MUST ADMIT!',
    },
    {
        button: '4/5, CONTINUE TO 5',
        text: 'Working on that home assignment was one of the best fun I had lately. I really hope you like it. I put a lot of though and efforts.',
        title: 'OVERALL!',
    },
    {
        button: 'SAY HI AND PRESS GOODBYE',
        text: 'So play around with the UI and code and let me know what you think of it. I want to thank you for the opportunity to show my abilities. SO, SEE YA :)',
        title: 'THIS IS THE LAST ONE MANY THANKS!',
    },
]

export const messagesExamples = [
    {
        button: 'CONTINUE',
        text: 'Roger is released from jail thanks to the proof you found in his phone!',
        title: 'YOU DID IT!',
    },
    {
        button: 'CONTINUE READING',
        text: '“Call me Ishmael. Some years ago—never mind how long precisely—having little or no money in my purse, and nothing particular to interest me on shore, I thought I would sail about a little and see the watery part of the world..."',
        title: 'MOBY DICK',
    },
    {
        button: 'OK GOT IT',
        text: 'Joe has just message you. The message contains a malware. DON\'T OPEN IT!',
        title: 'MESSAGE ALERT',
    },
    {
        button: 'START THE MISSION',
        text: 'Welcome, digital explorer! Your mission is to navigate the vast digital landscape of the virtual world through your simulated mobile phone. As you embark on this journey, you\'ll encounter various challenges and tasks that mimic real-life scenarios, from managing social media accounts to solving puzzles and interacting with virtual friends.',
        title: 'THE MISSION!',
    },

]

export const cancelMessages = [
    {
        button: 'REMIND ME!',
        text: 'You just missed something! Want to see it again?',
        title: 'WHOOPSIE!',
        allowCancel: true,
    },
    {
        button: 'REVEAL AGAIN',
        text: 'You\'re missing out! Do you want to take another look?',
        title: 'OH SNAP!',
        allowCancel: true,
    },
    {
        button: 'ONE MORE TIME',
        text: 'Wait! Did you mean to cancel that? Want to see it once more?',
        title: 'OOPSIE DAISY!',
        allowCancel: true,
    },
    {
        button: 'SHOW AGAIN',
        text: 'Hold up! You cancelled something. Wanna see it again?',
        title: 'HEY THERE!',
        allowCancel: true,
    },
    {
        button: 'REPLAY IT!',
        text: 'Wait a second! Want to catch that again?',
        title: 'IN A JIFFY!',
        allowCancel: true,
    }
]
